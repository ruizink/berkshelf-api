...
class GitlabClient
  attr_reader :endpoint, :private_token, :options
  ...
  #   very verbose!
  def initialize(endpoint, private_token, options = {}, logger = nil)
    @endpoint, @private_token, @options, @logger = endpoint, private_token, options, logger
  end
  ...
  # @return [Array<Hash>] the remote JSON data
  # @api private
  def _branches_for_project_id(project_id)
  ...
  def connection
    ...
    @connection ||= Faraday.new(endpoint,  options) do |faraday|
  ...
  class Project
    ...
    # @return [Array<String>]
    def branches
      client._branches_for_project_id(id).map { |h| h['name'] }
    rescue NoSuchProject
      []
    end
